...
export default function SetsList() {
  ...
  return (
    <div className="page-container">
      <h1 className="page-title font-handdrawn text-7xl mb-6">Paket Kartu (Sets)</h1>
      ...
      <div className="mb-10 flex flex-col md:flex-row gap-4 items-center bg-dutch-white p-4 border-[3px] border-black-coral shadow-[6px_6px_0_var(--color-black-coral)] rotate-[-1deg] hand-drawn z-10 relative">
        ...
        <input 
          ...
          placeholder="Cari nama ekspansi/pack..."
            ...
            return (
              <motion.div key={set.id} variants={{ hidden: { opacity: 0, scale: 0.9 }, visible: { opacity: 1, scale: 1 } }}>
                ...
                >
                  ...
                  <p className="text-center font-bold text-[10px] text-rhythm-blue uppercase tracking-widest mt-1 border-t-2 border-dashed border-black-coral/30 pt-2">{set.cardCount?.total || '?'} Kartu</p>
                </Link>
              </motion.div>
            );
          })}
          {filteredSets?.length === 0 && (
             <div className="col-span-full text-center text-super-dark py-24 font-handdrawn font-black text-4xl transform -rotate-2">"{q}" {t('not_found') || 'Tidak Ditemukan'}</div>
          )}
        </motion.div>
      )}
    </div>
